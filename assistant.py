...
class GenericAssistant(IAssistant):
    ...
    def train_model(self):
        ...
        self.words = [self.lemmatizer.lemmatize(w.lower()) for w in self.words if (w not in ignore_letters and w not in stopwords.words('italian'))]
    ...
    def _predict_class(self, sentence):
        ...
        print_res = [[i, r] for i, r in enumerate(res) if r > 0.05]
        print_res.sort(key=lambda x: x[1], reverse=True)

        ret_res =[]
        for r in print_res:
            ret_res.append({'intent': self.classes[r[0]], 'probability': str(r[1])})
        print(ret_res)
    ...
    def request(self, message):
        ...
        if ints == []:
            ints = [{'intent': 'none', 'probability': '1'}]
